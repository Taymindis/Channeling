...
class SSLSocketChannel extends SocketChannel {
    ...
    private static Logger logger = LoggerFactory.getLogger(SSLSocketChannel.class);
    ...
    synchronized public int write(ByteBuffer applicationBuffer) throws IOException, IllegalArgumentException {
        ...
        if (writtenToChannel < 0) {
            logger.debug("write: channel closed");
            return writtenToChannel;
        } else {
            int totalWritten = applicationBuffer.position() - intialPosition;
            logger.debug("write: total written: " + totalWritten + " amount available in network outbound: " + applicationBuffer.remaining());
            return totalWritten;
        }
    }
    ...
    protected int getMinApplicationBufferSize() {
        return sslEngineBuffer.getMinimumApplicationBufferSize();
    }
}